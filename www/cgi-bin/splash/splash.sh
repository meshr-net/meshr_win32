...
$(uci -q get luci_splash.general.redirect_url) || {
	set -x
	touch $meshr/var/state/luci_splash_locations
	touch $meshr/etc/config/luci_splash_locations
	MAC=$(grep "$REMOTE_HOST" $meshr/proc/net/arp | awk '{print $4}')
	uci -P $meshr/var/state set luci_splash_locations.${MAC//:/}=redirect
	uci -P $meshr/var/state set luci_splash_locations.${MAC//:/}.location="http://${HTTP_HOST}${REQUEST_URI}"
	set +x
}
