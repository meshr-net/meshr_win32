...
if "%NetConnectionID:""=%"=="" for /f "tokens=2* delims==" %%a in ('type %meshr:/=\%\var\run\wifi.txt ^| find "Caption="') do for /f "tokens=*" %%f in ('wmic path win32_networkadapter where Caption^="%%a" get NetConnectionID /value ^| find "="') do set "%%f"
