...
hotplug_dev() {
	env -i ACTION=$1 INTERFACE=$2 $meshr/sbin/hotplug-call net
}
...
insert_modules() {
	[ -d $meshr/etc/modules.d ] && {
		cd $meshr/etc/modules.d
		sed 's/^[^#]/insmod &/' $* | ash 2>&- || :
	}
}
...
find_mtd_index() {
	local PART="$(grep "\"$1\"" $meshr/proc/mtd | awk -F: '{print $1}')"
...
find_mtd_part() {
	...
	[ -d $meshr/dev/mtdblock ] && PREFIX=/dev/mtdblock/
...
find_mtd_chardev() {
	...
	[ -d $meshr/dev/mtd ] && PREFIX=/dev/mtd/
	echo "${INDEX:+$PREFIX$INDEX}"
}
...
uci_apply_defaults() {
	cd $meshr/etc/uci-defaults || return 0
	files="$(ls)"
	[ -z "$files" ] && return 0
	mkdir -p $meshr/tmp/.uci
...
group_add() {
	...
	[ -n "$IPKG_INSTROOT" ] || lock $meshr/var/lock/group
	echo "${name}:x:${gid}:" >> ${IPKG_INSTROOT}/etc/group
	rc=$?
	[ -n "$IPKG_INSTROOT" ] || lock -u $meshr/var/lock/group
	return $rc
}
...
user_add() {
	...
	local gid="${3:-$2}"
	local desc="${4:-$1}"
	local home="${5:-$meshr/var/run/$1}"
	local shell="${6:-$meshr/bin/false}"
	local rc
	[ -f "${IPKG_INSTROOT}/etc/passwd" ] || return 1
	[ -n "$IPKG_INSTROOT" ] || lock $meshr/var/lock/passwd
	echo "${name}:x:${uid}:${gid}:${desc}:${home}:${shell}" >> ${IPKG_INSTROOT}/etc/passwd
	echo "${name}:x:0:0:99999:7:::" >> ${IPKG_INSTROOT}/etc/shadow
	rc=$?
	[ -n "$IPKG_INSTROOT" ] || lock -u $meshr/var/lock/passwd
	return $rc
}
...
pi_include() {
	if [ -f "/tmp/overlay/$1" ]; then
		...
		if [ -n "$(ls $meshr/tmp/overlay/$1/*.sh 2>/dev/null)" ]; then
			for src_script in $meshr/tmp/overlay/$1/*.sh; do
				. "$src_script"
			done
		fi
	elif [ -d "$1" ]; then
...
pivot() { # <new_root> <old_root>
	mount -o noatime,move $meshr/proc $1/proc && \
	pivot_root $1 $1$2 && {
...
[ -z "$IPKG_INSTROOT" -a -f $meshr/lib/config/uci.sh ] && . $meshr/lib/config/uci.sh
