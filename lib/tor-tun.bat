...
if not "%torIP%"=="" set torIP=%torIP: =%
echo -"%torIP%" | find "." && ( curl -m 20 --proxy socks5h://%torIP%:9150 http://74.125.224.72 -o NUL && goto :torok )
:try_gateway
...
if not "%torIP%"=="127.0.0.1" (( curl -m 10 --proxy socks5h://%torIP%:9150 http://74.125.224.72 -o NUL || nc -z %torIP% 9150 ) && (
  echo ok
