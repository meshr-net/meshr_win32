...
function createindex()
   ...
   if luci.util.copcall(require, "luci.fastindex") and not smode then
...
function createindex_plain(path, suffixes)
   ...
   for _, suffix in ipairs(suffixes) do
      if smode then
      nixio.util.consume((fs.glob(path .. "freifunk/freifunk.lua")), controllers)
      nixio.util.consume((fs.glob(path .. "splash/*.lua")), controllers)
      else
   ...
   if indexcache then
      ...
      if cachedate then
         ...
         if cachedate > realdate then
            assert(
               sys.process.info("uid") == fs.stat(indexcache, "uid") or 1 -- for nfs mount
