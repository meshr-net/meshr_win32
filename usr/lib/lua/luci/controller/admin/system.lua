...
function action_packages()
   ...
   -- Letter
   local letter = string.byte(luci.http.formvalue("letter") or "*", 1)
   letter = (letter == 35 or letter == 42 or (letter >= 65 and letter <= 90)) and letter or 65
  ...
  no_lists, old_lists = ipkg.list_state()
--   local lists_dir = hostos:sub(1,3) == 'win' and '/usr/lib/ipkg/lists/' or "/var/opkg-lists/"
--   local tmp = nixio.fs.dir(lists_dir)
--   if tmp then
--      for tmp in tmp do
--         no_lists = false
--         tmp = nixio.fs.stat(lists_dir..tmp)
...
--   end
   ...
   -- Remove index cache
   if changes then
      nixio.fs.unlink(rootfs .. "/tmp/luci-indexcache")
   end
end
...
function action_flashops()
   ...
   local image_tmp   = rootfs .. "/tmp/firmware.img"
