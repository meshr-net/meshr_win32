...
function init(cursor)
   ...
   local n, i
   local ifs = nxo.getifaddrs(); 
   if hostos:sub(1,3) == 'win' then
      ifs = {}
      local out = utl.exec(rootfs .. '/usr/sbin/olsrd -int')
      for name in string.gmatch(out, "if[^:\r]+:[^\r]+") do
        ifs[#ifs+1] = {["name"] = name}
      end
   end
   for n, i in ipairs(ifs) do
      ...
      if _tunnel[name] or not _iface_ignore(name) then
         ...
         if hostos:sub(1,3) == 'win' then 
            _interfaces[name].ipaddrs[1] = i.name:match("%d+%.%d+%.%d+%.%d+")
         end  
         if prnt then
            _switch[name] = true
            _switch[prnt] = true
...
function interface._ubus(self, field)
   if not _ubusdevcache[self.ifname] and not hostos:sub(1,3) == 'win' then
...
function interface.get_i18n(self)
   if self.wif then
      ...
      local ips = self.dev and self.dev.ipaddrs[1];
      return "%s: %q %s" %{ self:get_type_i18n(), self:name() , ips or ''}
   end
end
...
if exts and hostos:sub(1,3) == 'win' then
   local ext
   for ext in exts do
