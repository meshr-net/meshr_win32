...
uci:foreach("wireless", "wifi-device", function(section)
	...
	if hwtype == "atheros" then
		local cc = hostos:sub(1,3) == 'win' and 0 or util.trim(sys.exec("grep -i '" .. syscc .. "' /lib/wifi/cc_translate.txt |cut -d ' ' -f 2"))
	...
	if hwtype and sys.call("/usr/bin/meshwizard/helpers/supports_vap.sh " .. device .. " " .. hwtype) == 0 then
		supports_vap = 1
	end
