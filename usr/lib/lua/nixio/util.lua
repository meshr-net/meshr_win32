...
function meta.writeall(self, data)
	...
	if not sent or not tonumber(sent) then
		return nil, code, msg, 0
	end

	local total = sent 

	while total and total < #data do
	...
	return function(flush)
		...
		while not line do
			...
			if line then
				bpos = endp
				return line
			elseif limit and bpos and #buffer and #buffer < limit + bpos then
        local N = limit + bpos - #buffer
				local newblock, code, msg = self:read(N)
				if not newblock or msg then
					return nil, code, msg
