...
. $dir/functions.sh
#set -x
#echo > $meshr/etc/config/olsrd

# Clean the config, remove interface wlan
handle_interface() {
        config_get interface "$1" interface
        if [ "$interface" = "wlan" ]; then
	echo	 uci delete olsrd.$1
        fi
}
config_load olsrd
...
uci_commitverbose "Setup new InterfaceDefaults" olsrd
...
#uci batch << EOF
uci set olsrd.olsrd_nameservice=LoadPlugin
uci set olsrd.olsrd_nameservice.library="olsrd_nameservice.so.0.3"
uci set olsrd.olsrd_nameservice.latlon_file="$meshr/var/run/latlon.js"
uci set olsrd.olsrd_nameservice.lat="$(uci -q get system.system.latitude)"
uci set olsrd.olsrd_nameservice.lon="$(uci -q get system.system.longitude)"
uci set olsrd.olsrd_nameservice.hosts_file="$meshr/var/etc/hosts.olsr"
uci set olsrd.olsrd_nameservice.sighup_pid_file="$meshr/var/run/dnsmasq.pid"
uci set olsrd.olsrd_nameservice.services_file="$meshr/var/run/services_olsr"
uci set olsrd.olsrd_nameservice.suffix="$suffix"
#EOF
...
# Setup watchdog
#uci set olsrd.olsrd_watchdog=LoadPlugin
#uci set olsrd.olsrd_watchdog.library="olsrd_watchdog.so.0.1"
#uci set olsrd.olsrd_watchdog.file="$meshr/var/run/olsrd.watchdog"
#uci set olsrd.olsrd_watchdog.interval=30

#uci_commitverbose "Setup olsr watchdog plugin" olsrd

# Setup jsoninfo
uci set olsrd.olsrd_jsoninfo=LoadPlugin
uci set olsrd.olsrd_jsoninfo.library="olsrd_jsoninfo.so.0.0"
uci_commitverbose "Setup olsr jsoninfo plugin" olsrd

# Setup olsrd_txtinfo
uci set olsrd.olsrd_txtinfo=LoadPlugin
uci set olsrd.olsrd_txtinfo.library="olsrd_txtinfo.so.0.1"

uci_commitverbose "Setup olsr olsrd_txtinfo plugin" olsrd
