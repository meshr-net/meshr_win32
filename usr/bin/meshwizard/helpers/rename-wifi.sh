...
IBwifis="$(uci show meshwizard.netconfig | grep 'IB_' | sed 's/meshwizard.netconfig\.\(IB_wifi.*\)_.*/\1/' | sort -u)"
...
pos=0
syswifis="$(uci show wireless |grep wifi-device | sed 's/wireless\.\(.*\)=.*/\1/' | sort -u)"
...
uci_commitverbose "Renaming wifi-devices in $meshr/etc/config/meshwizard" meshwizard
