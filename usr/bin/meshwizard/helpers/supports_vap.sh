...
if [ "$type" = "atheros" ]; then
	...
	if [ ! -x $meshr/usr/sbin/hostapd ]; then
		echo "WARNING: hostapd[-mini] is required to be able to use VAP with mac80211."
		exit 1
	fi
        # get driver in use
        netindex="$(echo $dev |sed 's/[a-zA-z]*//')"
	if [ -d /sys/class/net/wlan${netindex}$meshr/tmpice/driver/module ]; then
	        driver="$(basename $(ls -l /sys/class/net/wlan${netindex}$meshr/tmpice/driver/module | sed -ne 's/.* -> //p'))"
