...
if [ -z "${addnhosts/\var\$meshr/etc\/hosts.olsr/}" ]; then
	uci add_list dhcp.dnsmasq.addnhosts="$meshr/var/etc/hosts.olsr"
	if [ "$ipv6_enabled" = 1 ]; then
		uci add_list dhcp.dnsmasq.addnhosts="$meshr/var/etc/hosts.olsr.ipv6"
	fi
fi
