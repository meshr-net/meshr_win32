...
if "%1"=="stop" if not "%3"=="conn" for %%E in (%bin%git %bin%sh %bin%lua %bin%uci %bin%sleep %bin%start-stop-daemon) do wmic process where ExecutablePath='%%E.exe' delete

if "%3"=="conn" (
  call %meshr:/=\%\lib\setip.bat %meshr:/=\%\var\run\wifi.txt
