...
git pull origin master < NUL || ( 
  ...
  git remote set-url origin https://github.com/meshr/meshr_x86
  ...
  git pull origin master < NUL > tmp\git.log || (
      ...
      bin\git reset --hard origin/master < NUL || ( sudo cmd /c %meshr:/=\%\bin\services.bat stop
        sudo -b cmd /c "cd %meshr% && bin\git reset --hard origin/master < NUL"
        sleep 9 )
